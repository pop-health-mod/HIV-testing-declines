    ...
    for (n in 1:n_year) {
      ...
      if (age == '15-24') {
        ...
        test_rate_t <- rbind(
          do.call(
            "rbind",
            replicate(ind15, test_rate15, simplify = FALSE)
          ),
        ...
        mort_rate_t <- rbind(
          do.call(
            "rbind",
            replicate(ind15, mort_rate15, simplify = FALSE)
          ),
      ...
      if (age == '25-34') {
        ...
        test_rate_t <- rbind(
          do.call(
            "rbind",
            replicate(ind25, test_rate25, simplify = FALSE)
          ),
          do.call("rbind", replicate((ind35 - ind25), test_rate35, simplify = FALSE
          )),
          do.call("rbind", replicate((vec_l - ind35), test_rate50, simplify = FALSE
        ...
        mort_rate_t <- rbind(
          do.call(
            "rbind",
            replicate(ind25, mort_rate25, simplify = FALSE)
          ),
          do.call("rbind", replicate((ind35 - ind25), mort_rate35, simplify = FALSE
          )),
          do.call("rbind", replicate((vec_l - ind35), mort_rate50, simplify = FALSE
      ...
      if (age == '35-49') {
        ...
        test_rate_t <- rbind(do.call(
          "rbind",
          replicate(ind35, test_rate35, simplify = FALSE)
        ),
        do.call("rbind", replicate((vec_l - ind35), test_rate50, simplify = FALSE
        )))
        mort_rate_t <- rbind(do.call(
          "rbind",
          replicate(ind35, mort_rate35, simplify = FALSE)
        ),
        do.call("rbind", replicate((vec_l - ind35), mort_rate50, simplify = FALSE
        )))
      }
      if (age == '50-99') {
        test_rate_t <- do.call("rbind",
                               replicate(vec_l, test_rate50, simplify = FALSE))
        mort_rate_t <- do.call("rbind",
                               replicate(vec_l, mort_rate50, simplify = FALSE))
      }
      ...
      # this is wrong - using it for AHD - some later work uses indexing rather than naming
      # which may cause bugs if i add a new index: here is proper line
      # val$prb350cd4[val$year == year[n]] <- sum(nb_dx[ , 1:2]) / N
      val$prb350cd4[val$year == year[n]] <- sum(nb_dx[, 1:4]) / N
...
denom_lifetable <- function(mod, fp, df_ind) {
  ...
  for (i in seq_along(df_ind$haidx)) {
    ...
    if (df_ind$hvidx[i] %in% c(0, 1)) {
      # testing among HIV-
      
      pop_hivn_ha <- apply(mod[paidx, sidx, fp$ss$hivn.idx, df_ind$yidx[i], drop = FALSE], 2:3, fastmatch::ctapply, fp$ss$ag.idx[paidx], sum)
      
      tested_ha <- attr(mod, "testnegpop")[haidx, sidx, fp$ss$hivn.idx, 1, df_ind$yidx[i], drop = FALSE]
      
      tested_anc_ha <- attr(mod, "testnegpop")[haidx, sidx, fp$ss$hivn.idx, 2, df_ind$yidx[i], drop = FALSE]
    ...
    val <- data.frame(df_ind[, !(
      colnames(df_ind) %in% c(
        "agestart",
        "aidx",
        "agspan",
        "haidx",
...
pool_time_dx_cr <- function(mod,
  ...
  group_lst <- vector("list", n_year)
  
  for (n in 1:n_year) {
    ind_yr <- year[n] - fp$ss$proj_start + 1
    
    time_mat <- expand.grid(
      year = year[n],
      age = c("15-24", "25-34", "35-49", "50-99"),
      sex = c("male", "female"),
      test_ever = c("never", "ever")
    )
    time_mat <- rbind(time_mat,
                      expand.grid(
                        year = year[n],
                        age = c("15-24", "25-34", "35-49", "50-99"),
                        sex = "female",
                        test_ever = c("ever_anc")
                      ))
    ...
    index_age <- data.frame(x = c(1, 11, 21, 36), y = c(10, 20, 35, 66))
    
    info_need <- expand.grid(
      year = year[n],
    ...
    for (i in 1:nrow(time_mat)) {
      if (time_mat$age[i] == "15-24") {
        index_agei <- index_age[1, ]
      }
      if (time_mat$age[i] == "25-34") {
        index_agei <- index_age[2, ]
      }
      if (time_mat$age[i] == "35-49") {
        index_agei <- index_age[3, ]
      }
      if (time_mat$age[i] == "50-99") {
        index_agei <- index_age[4, ]
      }
      if (time_mat$sex[i] == "male") {
        index_sexi <- 1
      }
      if (time_mat$sex[i] == "female") {
        index_sexi <- 2
      }
      # optimization
      if (!(time_mat[i, ]$age %in% age &
            time_mat[i, ]$sex %in% sex)) {
        next
      }
      
      
      
      result_i <- time_to_dx_cr(
        fp,
        year = year[n],
        age = as.character(time_mat$age[i]),
      ...
      if (time_mat$test_ever[i] == 'never') {
        time_mat$w[i] <- sum(fp$infections[index_agei$x:index_agei$y, index_sexi, ind_yr]) *
          denom$w_n[denom$agegr == time_mat$age[i] &
                      denom$sex == time_mat$sex[i]]
      } else if (time_mat$test_ever[i] == 'ever') {
        time_mat$w[i] <- sum(fp$infections[index_agei$x:index_agei$y, index_sexi, ind_yr]) *
          denom$w_e[denom$agegr == time_mat$age[i] &
                      denom$sex == time_mat$sex[i]]
      } else if (time_mat$test_ever[i] == 'ever_anc') {
        time_mat$w[i] <- sum(fp$infections[index_agei$x:index_agei$y, index_sexi, ind_yr]) *
          denom$w_ae[denom$agegr == time_mat$age[i] &
                       denom$sex == time_mat$sex[i]]
      }
      
    }
    time_mat_sel <- time_mat[time_mat$age %in% age &
                               time_mat$sex %in% sex, ]
    ...
    group_lst[[n]] <- time_mat_sel          # <-- add this line
    
    }
  
  groups_all <- data.table::rbindlist(group_lst)
  
  # collapse test_ever within each year x age x sex, weighting as in the pooled value
  groups <- groups_all[, .(
    time_dx_avg = weighted.mean(time_avg, w),
    time_dx_med = weighted.mean(time_med, w),
    prb6mo      = weighted.mean(prb6,   w),
    prb1yr      = weighted.mean(prb1,   w),
    prb2yr      = weighted.mean(prb2,   w),
    prb5yr      = weighted.mean(prb5,   w),
    prb500cd4   = weighted.mean(prb500, w),
    prb350cd4   = weighted.mean(prb350, w),
    propdx      = weighted.mean(prop,   w),
    w           = sum(w)
  ), by = .(year, age, sex)]
  
  return(list(pooled = val, groups = groups))

  }

simul_time_dx_cr <- function(samp,
                             mod,
                             fp,
                             pmtct,
                             hivdemo_proj,
...
simul_pool_time_dx_agg_prev <- function(samp,
                                        mod,
                                        fp,
                                        year = c(2000:2018),
                                        age = c("15-24", "25-34", "35-49", "50-99"),
  ...
  if (all(age %in% c("15-24", "25-34", "35-49", "50-99")) &
      length(unique(age)) == 4) {
    index_age <- 1:66
    age_out_time_ind <- "15-99"
  }
  ...
  if (parallel == T) {
    ...
    fun_samp_par <- function(i,
      ...
      result_i$pooled$sampi <- i
      result_i$groups$sampi <- i
      return(result_i)
    ...
    fun_samp_par <- function(i,
      ...
      result_i <- pool_time_dx_cr(mod_local, fp, year = year, age = age, sex = sex)
      result_i$pooled$sampi <- i
      result_i$groups$sampi <- i
      return(result_i)
    }
  ...
  pooled_lst <- lapply(val_lst, `[[`, "pooled")
  groups_lst <- lapply(val_lst, `[[`, "groups")
  
  pooled <- data.table::rbindlist(pooled_lst)
  pooled <- merge(pooled, poids, by = "year")
  pooled$age <- paste(age, collapse = "+")
  pooled$sex <- paste(sex, collapse = "+")
  
  groups <- data.table::rbindlist(groups_lst)   # keeps real age/sex and per-group w
  
  return(list(pooled = pooled, groups = groups))
  }
