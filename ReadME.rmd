...
This is an update to Shiny90 to include ANC estimates for the publication and the counterfactual to estimate the impacts of reductions in testing
----
this script was tested and is only intended to work on windows, the package used to parallel computations is not functional on mac.
...
runtime : 5 seconds
comment: for full PJNZ files use "ReadMe - real PJNZ"
----
...
----
goal: visualize model fits to survey data by age and sex and to program data by anc and non-anc
estimate: ~ 10 seconds 
comments: some error may be present due to stochastic runs of nelder-mead, if too different rerun prior 2 steps
...
comments: this uses the script 2.1 counterfactual creation rather than have a long series of code
outputs: counter factual files
----
...
estimate: 1191 (~20 minutes) - 2077 (~40 minutes) seconds seconds
...
comments2: if you have an error message about core usage, manually adjust the parallel::detectcores() line, leave at least 1 core free, at the moment it uses 3/4 of cores on the system
