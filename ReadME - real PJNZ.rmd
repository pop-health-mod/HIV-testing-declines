...
source(here::here("anc testing/0.7 graphing functions.R"))
...
# we initally perform descriptive plotting with the first 90 package

# All PJNZ files are avilible publically but must be obtained from aidsinfo@unaids.org
# example : "Malawi_2024_v11_ART_Num.pjnz"


# requird befroe running: 
# master survey file
# survey_hts_master.rda
# list of PJNZ files (updated for 2024 files)

# creates and stores PMTCT and HIV demoproj files in here::here(data)
source("anc testing/0.2 create PMTCT and HIVdemoproj.R")
# creates and stores make country presimul files in here::here(data)
source("anc testing/0.3 creating make country.R")

# this finds all pjnz files(careful it will locate them in all dictionaries)
# and provide a full link as needed
pjnz_list = list.files(pattern = "\\.pjnz$", full.names = TRUE,recursive = T,ignore.case = T)

# we are using the 30th file as it is Malawi in my case, adjust as needed
pjnz <- pjnz_list[30]
...
# to appropriatly model a country both survey and program data are required


# if you have a set of PJNZ files, "0.3 creating make country" will automaticall extract these

# here a synthetic data point and program point are created as an exampl
age_group <- c('15-24','25-34','35-49')

dat <- first90::select_hts(make_country$Malawi$survey_hts, cnt, age_group)

prg_dat <- first90::select_prgmdata(make_country$Malawi$prgm_dat, cnt, age_group)

# We visualize the program data
first90::plot_inputdata(prg_dat, fp)
```

The following functions enable users to produce individual plots.
```{r seperate program}
...
# save the file
# saveRDS(object = make_country, file = here::here('anctesting/data/make_country_simul.rds'))
...
source("anc testing/0.7 graphing functions.R")

# this should have the full malawi simulation file from the prior steps
survey_fit_all(make_country,cnt = "Malawi")

survey_fit_postive(make_country,cnt = "Malawi")
...
source("anc testing/2.1 counterfactual creation.R")
# this fucntion resets cnt need to readd malawi
cnt = "Malawi"

#load the files
counter_years <- readRDS("anc testing/data/counter_years.rds")
counter_anc_years <- readRDS("anc testing/data/counter_years.rds")
